...
for i in range (0, len(valid)):
	if (not valid[i]['relation'] == getKeyFromValue(generatedText, testResult[i])):
		invalids.append(valid[i])
		rel = {}
		rel['head'] = valid[i]['h']['name']
		rel['tail'] = valid[i]['t']['name']
		rel['sentence'] = ' '.join(str(x) for x in valid[i]['token'])
		rel['realRelation'] = valid[i]['relation']
		rel['predictedRelation'] = getKeyFromValue(generatedText, testResult[i])
		invalidRelations.append(rel)
	else:
		counter += 1

savedFile = open("gold_data_predicted_invalidRelations_withSpadTrain.json", "w", encoding='utf-8')
savedFile.write(str(json.dumps(invalidRelations, indent=4, sort_keys=True, ensure_ascii=False)))
savedFile.close()
